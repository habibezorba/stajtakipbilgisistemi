...
<body>
	...
	<div class="container col-md-5">
		<div class="card">
			<div class="card-body">
				<c:if test="${stajdefteri != null}">
					<form action="stajdefteriGuncelle" method="post">
				</c:if>
				<c:if test="${stajdefteri == null}">
					<form action="stajdefteriEkle" method="post">
				</c:if>

				<caption>
					<h2>
						<c:if test="${stajdefteri != null}">
            			Staj Defteri Bilgilerini Düzenle
            		</c:if>
						<c:if test="${stajdefteri == null}">
            			Yeni Staj Defteri Ekle
            		</c:if>
					</h2>
				</caption>

				<c:if test="${stajdefteri != null}">
					<input type="hidden" name="id" value="<c:out value='${stajdefteri.id}' />" />
				</c:if>

				
				<fieldset class="form-group">
					<label>Staj Başvurusu</label>
					<select name="stajbasvuru" id="stajbasvuru" class="form-control">
						<c:forEach var="sb" items="${listStajbasvuru}">
							<option value="${sb.id}">${sb.sirket.ad} ${sb.baslangicTarihi} - ${sb.bitisTarihi}</option>
						</c:forEach>
					</select>
				</fieldset>
				
				<fieldset class="form-group">
					<label>Defter</label> <input type="file"  accept="application/pdf,application/vnd.ms-excel"
						value="<c:out value='${stajdefteri.defterPath}' />" class="form-control"
						name="defter" required="required">
				</fieldset>
				...
				<c:if test="${loggedOgrenci != null }">
					...
					<c:if test="${stajdefteri != null }">
						<input type="number"
						value="<c:out value='${stajdefteri.kabulEdilenGunSayisi}' />" class="form-control"
						name="kabulGunSayisi">
					</c:if>
				</fieldset>
...
<script>
	(function() {
		var num = '' + '${stajdefteri.stajBasvuru.id}'
		if(Number(num) != 0)
			document.getElementById("stajbasvuru").value = Number(num)
