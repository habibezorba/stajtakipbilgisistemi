...
<body>
	...
	<div class="row">
		<div class="container">
			<h3 class="text-center" style="font-style: normal; color: white;">Staj Defteri Listesi</h3>
			<hr>
			<div class="container text-left">
			</div>				 <a href="<%=request.getContextPath()%>/yeniStajdefteri" class="btn btn-success" style="background-color:#ff5a00;">Yeni Staj Defteri Ekle</a>
			...
			<table class="table table-bordered" style="color: white;">
				...
				<tbody>
					<c:forEach var="stajDefteri" items="${listStajdefteri}">
						<tr>
							<td><c:out value="${stajDefteri.id}" /></td>
							<td><c:out value="${stajDefteri.stajBasvuru.ogrenci.ad}" /> <c:out value="${stajDefteri.stajBasvuru.ogrenci.soyad}" /></td>
							<td><c:out value="${stajDefteri.stajBasvuru.danisman.ad}" /> <c:out value="${stajDefteri.stajBasvuru.danisman.soyad}" /></td>
							<td><c:out value="${stajDefteri.stajBasvuru.baslangicTarihi}" /></td>
							<td><c:out value="${stajDefteri.stajBasvuru.bitisTarihi}" /></td>
							<td><c:out value="${stajDefteri.getKabulDurumuText()}" /></td>
							<td><c:out value="${stajDefteri.kabulEdilenGunSayisi}" /></td>
							<td><a target="_blank" href="<c:out value="${stajDefteri.defterPath}" />">Defter</a></td>
							<td><a style=" color:#00FF00;" href="stajdefteriDuzenle?id=<c:out value='${stajDefteri.id}' />">Düzenle</a>
								&nbsp;&nbsp;&nbsp;&nbsp; <a style=" color:#ee0000;"
								href="stajdefteriSil?id=<c:out value='${stajDefteri.id}' />">Sil</a></td>
						</tr>
					</c:forEach>
				</tbody>
